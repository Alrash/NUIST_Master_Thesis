...
\contentsappend           % 从contentsappend.txt导入目录追加内容（可选）
...
\addreferencetoctl          % 参考文献追加至目录，固定位置，否则页码错误
\bibliography{sample}
...
% 附录样式2 -- 附录: A、B...
\appendixtoc \\             % 修改目录显示的深度，即仅显示“附录...”
\appendixrenewenv \\        % 附录公式序号变为A-?，建议采用
\appendixchapter            % 样式2起始
...
\begin{figure}[htbp!]
    ...
    \\ 
    \subfigure[使用\textbackslash blindtrial $\&$ \textbackslash btstyle]{
        \begin{minipage}[t]{0.5 \linewidth}
            \centering
            \includegraphics[width = 1\linewidth, keepaspectratio]{figs/blindtrial2}
        \end{minipage}
    }
    \caption{三类封面示意图} \label{fig:title}
\end{figure}
